...
type require = B00_ocaml_lib.Name.t * smeta
type t =
  { file : Fpath.t;
...
let pp_dump ppf s =
  ...
  in
  let pp_reqs = Fmt.(list ~sep:sp (pp_fst B00_ocaml_lib.Name.pp)) in
  Fmt.record
...
let parse_require_directive d ~sbyte ~sline =
  string_to B00_ocaml_lib.Name.of_string (parse_string d)
