  ...
  | (l :: ls) as data ->
      ...
      | Some (dhex, dname) ->
          ...
          | exception Invalid_argument _ ->
              ...
              | false -> B0_lines.err n "%S: could not parse digest" dhex

  and parse_unit acc file defs deps ldeps name n = function
  | [] -> B0_lines.err n "unexpected end of input"
  ...
  module Deps = struct
    let of_string ?(file = Os.File.dash) ?src_root data =
      ...
      let parse_path n p = (* ocamldep escapes spaces as "\ ", a bit annoying *)
        ...
        | Error j -> B0_lines.err n "%d: illegal escape" j
        | Ok p ->
      ...
      let parse_line ~src_root n line acc =
        ...
        | None -> B0_lines.err n "cannot parse line: %S" line
        | Some (file, mods) ->
      ...
      in
      B0_lines.fold ~file data (parse_line ~src_root) Fpath.Map.empty
