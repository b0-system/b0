...
let get_b0_file_src c k =
  ...
  Result.bind (B0_file_src.of_string ~file:b0_file s) k

let boot c root =
  let pp_boots = Fmt.(list @@ hbox @@ list ~sep:sp (using fst string)) in
  get_b0_file_src c @@ fun src ->
  let boots = match root with
  | true -> Ok (B0_file_src.b0_boots src)
  | false ->
      Result.bind (B0_file_src.expand src) @@ fun e ->
      Ok (B0_file_src.expanded_b0_boots e)
  in
...
let edit c all =
  ...
  | true ->
      ...
      Result.bind (B0_file_src.expand src) @@ fun e ->
      let incs = B0_file_src.expanded_b0_includes e in
...
let includes c root details =
  ...
  | true -> Ok (B0_file_src.b0_includes src)
  | false ->
      Result.bind (B0_file_src.expand src) @@ fun e ->
      Ok (B0_file_src.expanded_b0_includes e)
  in
...
let source c root =
  ...
  | false ->
      ...
      Result.bind (B0_file_src.of_string ~file:b0_file s) @@ fun src ->
      Result.bind (B0_file_src.expand src) @@ fun e ->
      let esrc = B0_file_src.expanded_src e in
      Log.app (fun m -> m "%s" esrc);
      Ok B0_driver.Exit.ok

let requires c root =
  let pp_require = Fmt.using fst B0_ocaml_lib.Name.pp in
  get_b0_file_src c @@ fun src ->
  let reqs = match root with
  | true -> Ok (B0_file_src.requires src)
  | false ->
      Result.bind (B0_file_src.expand src) @@ fun e ->
      Ok (B0_file_src.expanded_requires e)
  in
