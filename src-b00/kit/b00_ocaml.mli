...
(** OCaml tools and toolchain configuration. *)
module Tool : sig
  ...
  (** {1:conf Toolchain configuration} *)
...
(** OCaml and C stub compilation.
    ...
    This module has basic compilation steps and abstract
...
module Compile : sig
  ...
  val c_to_o :
    ?post_exec:(B000.Op.t -> unit) -> ?k:(int -> unit) -> Memo.t -> Cmd.t ->
    reads:Fpath.t list -> c:Fpath.t -> o:Fpath.t -> unit
  (** [c_to_o m args ~reads ~c ~o args] compiles the C file [c] to the
      object file [o] with arguments [args]. It assumes the
      compilation depends on C include header files [reads] whose
      parent directories are added as [-I] options. *)

  val mli_to_cmi :
    ?post_exec:(B000.Op.t -> unit) -> ?k:(int -> unit) -> Memo.t -> Cmd.t ->
    reads:Fpath.t list -> mli:Fpath.t -> o:Fpath.t -> and_cmti:bool -> unit
  (** [mli_to_cmi m cmd ~reads ~mli ~o ~and_cmti] compiles the file
      [mli] to the cmi file [o] and, if [and_cmti] is [true], to the
      cmti file [Fpath.(o -+ ".cmti")] with arguments [args]. It
      assumes the compilation depends on cmi files [reads] whose
      parent directories are added as [-I] options. *)

  val ml_to_cmo :
    ?post_exec:(B000.Op.t -> unit) -> ?k:(int -> unit) -> Memo.t -> Cmd.t ->
    reads:Fpath.t list -> has_cmi:bool -> ml:Fpath.t -> o:Fpath.t ->
    and_cmt:bool -> unit
  (** [ml_to_cmo m args ~reads ~has_cmi ~ml ~o ~and_cmt] compiles the
      file [ml] to cmo file [o] and, if [and_cmt] is [true], to
      the cmt file [Fpath.(o -+ ".cmti")] with arguments [args].  It
      assumes the compilation depends on the cmi files [reads] whose
      parent directories are added as [-I] options. [has_cmi] indicates
  ...
  val ml_to_cmx :
    ...
    reads:Fpath.t list -> has_cmi:bool -> ml:Fpath.t -> o:Fpath.t ->
    and_cmt:bool -> unit
  (** [ml_to_cmx m args ~reads ~has_cmi ~ml ~o ~and_cmt] compiles the file
      [ml] to cmx file [o] and, if [and_cmt] is [true], to the
      cmt file [Fpath.(o -+ ".cmti")] with arguments [args]. It assumes
      the compilation depends on the cmi and cmx files [reads] whose
      parent directories are added as [-I] options. [has_cmi] indicates
      whether the [ml] file already has a corresponding cmi file, in which
      case it should be in [reads] (FIXME specify path directly ?). *)

  val ml_to_impl :
    ?post_exec:(B000.Op.t -> unit) -> ?k:(int -> unit) -> Memo.t ->
    code:code -> Cmd.t -> reads:Fpath.t list -> has_cmi:bool ->
...
module Archive : sig
  ...
  val cstubs :
    ...
    B00.Memo.t -> c_objs:Fpath.t list -> odir:Fpath.t ->
    oname:string -> unit
  (** [cstubs m ~args ~c_objs ~odir ~oname] creates in directory
  ...
  val native :
    ...
    Memo.t -> has_cstubs:bool -> cobjs:Fpath.t list -> odir:Fpath.t ->
    oname:string -> unit
  (** [native m ~args ~has_cstubs ~cobjs ~obase] creates in directory
      [odir] a native code archive named [oname] with the OCaml native
      code compilation objects [cobjs]. *)

  val code :
    ?post_exec:(B000.Op.t -> unit) -> ?k:(int -> unit) -> ?args:Cmd.t ->
    Memo.t -> code:Compile.code -> has_cstubs:bool -> cobjs:Fpath.t list ->
...
module Link : sig
  ...
  val code :
    ?post_exec:(B000.Op.t -> unit) -> ?k:(int -> unit) -> ?args:Cmd.t ->
    Memo.t -> code:Compile.code -> c_objs:Fpath.t list -> cobjs:Fpath.t list ->
    o:Fpath.t -> unit
...
v} *)
  ...
  module Src : sig
    ...
    val v :
      ...
      mli_deps:Name.Set.t -> ml:Fpath.t option ->
      ml_deps:Name.Set.t -> t
    ...
    val impl_file : code:Compile.code -> in_dir:Fpath.t -> t -> Fpath.t
    ...
    val as_impl_dep_files :
      ?init:Fpath.t list -> code:Compile.code -> in_dir:Fpath.t -> t ->
      Fpath.t list
    ...
    val of_srcs :
      Memo.t -> srcs:Fpath.t list -> src_deps:Name.Set.t Fpath.Map.t ->
      t Name.Map.t
    ...
    val find_local_deps :
      t Name.Map.t -> Name.Set.t -> t list * Name.Set.t
...
(** OCaml compiled objects. *)
module Cobj : sig

  val archive_ext_of_code : Compile.code -> Fpath.ext
  (** [archive_ext_of_code c] is [.cma] or [.cmxa] according to [c]. *)

  val object_ext_of_code : Compile.code -> Fpath.ext
...
(** Libraries
    ...
    interfaces and object files. OCaml libraries are found by name
...
module Lib : sig
  ...
  val of_dir :
    ...
    dir:Fpath.t -> archive:string option -> (t, string) result
...
(** Library resolvers.
    ...
    libraries as per the OCaml library convention and falls back on
    [ocamlfind] if that fails.
