...
module Client : sig
  ...
  val request :
    t -> follow:bool -> Request.t -> (Response.t, string) result
  (** [request httpc ~follow r] performs request [r] via [httpr].  If [follow]
      ...
      The response's {!Response.headers} are lowercased. *)

  val head_request_follow_location :
    ?headers:headers -> t -> Net.Url.t -> (Net.Url.t, string) result
  ...
  val curl_fetch_args :
    ?args:Cmd.t -> progress:bool -> Net.Url.t -> Filepath.t -> Cmd.t
