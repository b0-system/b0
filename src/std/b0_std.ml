...
module Cmd = struct
  ...
  (* Parsing is loosely based on
    ...
    with Failure err -> Fmt.error "command line %a: %s" Fmt.OCaml.string s err
...
module Os = struct
  ...
  module Fd = struct
    ...
    let read_file file fd =
      ...
      with
      | Failure e -> Fmt.failwith_notrace "%s: %s" file e
  ...
  module Fs_base = struct
    ...
    let dir_create ?(mode = 0o755) ~make_path dir =
      ...
      try
        ...
        | Unix.Unix_error (Unix.EEXIST, _, _) ->
            if is_dir (Fpath.to_string dir) then Ok false else
    ...
    let rec handle_force_open_fdout
      ...
      match Unix.openfile (Fpath.to_string file) fls mode with
      | fd -> Ok fd
  ...
  module Tmp = struct
    ...
    let path ?(make_path = true) ?dir ?(name = format_of_string "tmp-%s") () =
      ...
      | n ->
          ...
          | exception Unix.Unix_error (Unix.ENOENT, _, _) -> Ok (Fpath.v file)
  ...
  module File = struct
    ...
    let is_executable file = is_executable' (Fpath.to_string file)
  ...
  module Dir = struct
    ...
    let prune_denied _ _ p _ =
      try (Unix.access (Fpath.to_string p) Unix.[R_OK; X_OK]; false) with
  ...
  module Cmd = struct
    ...
    let run_status_out
      ...
      try
        ...
        let out = Fd.read_file (Fpath.to_string tmpf) fd in
        Tmp.rem_file tmpf;
    ...
    let execv ?env ?cwd ?argv0 cmd =
      let err_execv f e = Fmt.error "execv %s: %s" f e in
      match Cmd.to_list cmd with
