...
(* Parsing is loosely based on
  ...
  try
    ...
    let parse_squoted s =
      let tok, rem = B0__string.span_while not_squote (tail s) in
    ...
    let rec loop acc s =
      let data, rem = B0__string.span_while is_data s in
    ...
    let parse_token s =
      ...
      | Some c ->
          let sat = tok_char in
          let tok, rem = B0__string.span_while sat s in loop (tok :: acc) rem
      in
      loop [] s
    in
