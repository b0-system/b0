...
(** [exit_of_result v] exits with {!B0_cli.Exit.ok} if [v] is [Ok ()] and
    logs the Error and exits with {!B0_cli.Exit.some_error} if [v]
    is [Error _]. *)
...
(** [exec_tool tool e args] executes the tool [exe] with arguments [cmd]
    ...
    with {!B0_cli.Exit.some_error}. *)

(** {1:cli Command line interaction}

    Use {!B0_cli} to parse cmdlet arguments and {!B0_cli.Exit} for
    exit codes. Given a suitable {!Cmdliner} term this function can be
    used to implement the cmdlet's command. *)
