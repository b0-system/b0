...
val make :
  b0_dir:Filepath.t -> build:B0_build.t -> cwd:Filepath.t ->
  root_dir:Filepath.t -> scope_dir:Filepath.t -> driver_env:Os.Env.t -> t
...
val unit_dir : t -> B0_defs.b0_unit -> Filepath.t
...
val dir : t -> dir -> Filepath.t
(** [dir env d] looks up [d] in [env], raises [Invalid_argument]
    on [`Unit_dir]. *)

(** {1:rel Relative file resolution} *)

val in_root_dir : t -> Filepath.t -> Filepath.t
(** [in_scope_dir env p] is [Filepath.(root_dir env // p)]. *)

val in_scope_dir : t -> Filepath.t -> Filepath.t
(** [in_scope_dir env p] is [Filepath.(scope_dir env // p)]. *)

val in_scratch_dir : t -> Filepath.t -> Filepath.t
(** [in_scope_dir env p] is [Filepath.(scratch_dir env // p)]. *)

val in_unit_dir : t -> B0_defs.b0_unit -> Filepath.t -> Filepath.t
(** [in_unit_dir env u p] is [Filepath.(unit_unit_dir env u // p)]. *)

val in_dir : t -> dir -> Filepath.t -> Filepath.t
(** [in_dir env d p] is [Filepath.(in_dir env d // p)]. *)
...
val unit_exe_file : t -> B0_defs.b0_unit -> (Filepath.t, string) result
