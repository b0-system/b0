...
(** OCaml compiler code generation specification.
    ...
    In an OCaml build, the codes that are generated must be precisely
    known because the OCaml compilers compete to produce shared build
    artefacts. Besides, the code that needs to be generated for a unit
    depends on how it will be used. Here are a few examples:

    {ul
    {- When compiling a library for installation, all codes supported by
       the installation must be generated.}
    {- When compiling an executable tool only one code needs to be generated.
       For example most often native code if available and bytecode if not.}
    {- For a browser executable and considering {!B0_jsoo}, only the bytecode
       must be generated. Native code compilation is technically incompatible.}}

    The user of the build may also want to choose the codes to be
    generated. If unspecified, a reasonable default should be derived
    depending on the units that must build.

    We call {e built codes} the codes generated in a build. They are
    stored in {!Code.built} which OCaml build procedures should
    consult and adapt to.

    Build units can constrain the codes they want their build
    procedure to generate by defining the {!Code.restrict} function
    which given {!Code.built} must return which codes must be
    generated for the unit (or the empty set if they can't build).
    The result is used by the unit's build procedure to act accordingly.

    Now remains how to describe how {!Code.built} is determined. The
    user can specify a build desire with the {!Code.wanted} key, see its
...
module Code : sig
  ...
  | Byte (** OCaml bytecode. *)
  | Native (** Native code. *)
  | Wasm (** Wasm code, not available use {!B0_jsoo} instead. *)
  (** The type for codes. *)

  (** Sets of codes.

      {b Note.} These signatures are the same as those from {!Stdlib.Set.S}
      but we restrict access to the module in case we want to switch to
      something less heavy weight for sets of two or three elements. *)
  module Set : sig
    type code = t
    ...
    val min_elt : t -> code
    (** [min_elt s] is the minimal element in [s] note that if [Byte]
        is in [s] then this is [Byte]. *)

    val of_list : code list -> t
    (** [of_list cs] is a set containing exactly [cs]. *)

    val to_list : t -> code list
    ...
    (** [fold f s acc] folds [f] over the elements of [s] starting with
        [acc]. *)
  ...
  end with type code := t

  val none : Set.t
  (** [none] is {!Set.empty} *)

  val byte : Set.t
  (** [byte] is {!Set.singleton Byte} *)

  val native : Set.t
  (** [native] is {!Set.singleton Native} *)

  val wasm : Set.t
  (** [wasm] is {!Set.singleton Native} *)
  ...
  (** The type for codes restrictions. Given a set of codes
      to generate, indicates which ones to generate in a given context.
      The string is used for documentation. *)

  val restrict : restrict B0_meta.key
  (** [restrict] can be set on a unit to define which codes should
      should be generated for the unit. It is invoked by build
      procedures with the values of {!Code.built} and should return a
      subset of it. It default to {!Fun.id}.
      ...
      {- This should rather be a per unit {!B0_store.key}, determined
         once and user settable from the cli or a conf spec}} *)

  val unique_favour_native : restrict
  (** [unique_favour_native] returns a single code which is [Native] if in the
      given set and {!Set.min_elt} otherwise, that is [Byte] if it's in
      the set. *)

  val check_any : supported:Set.t -> by:B0_build.t -> unit Fut.t
  (** [check_any ~supported ~by:build] fails the current build unit if the
      intersection of [supported] and {!built} in [build] is empty.
      This should be used by build procedures to check they can be
      built. These empty intersections may happen on dynamic requests
      of may units. *)

  val check_all : supported:Set.t -> by:B0_build.t -> unit Fut.t
  (** [check_all] is like {!check_any} but checks for containement
      of [supported] in {!built}. *)

  (** {1:store_keys Store keys} *)

  type wanted =
  | Auto (** The set made of the union of calling all {!Code.restrict}
             of OCaml executable units that {e must} build with
             {!B0_ocaml.Conf.codes} or {!B0_ocaml.Conf.codes} itself
             if there is no such executable. *)
  | Wanted of t list (** These exact codes. *)
  (** The type for generated code wanted by the user. *)

  val wanted : wanted B0_store.key
  (** [wanted] indicates which codes should be generated. Defaults
      to [Auto]. *)

  val built : Set.t B0_store.key
  (** [built] is a memo key indicating the built codes. By default
      determines by consulting {!wanted}. It fails the build if
      the later is {!Wanted}[ w] and [w] is not included in
      {!B0_ocaml.Conf.codes}. *)
...
module Compile : sig
  ...
  (** [modsrc_intf m ~opts ~requires ~modsrcs ~and_cmti src]
      ...
      produced. If [src] has no [.mli] this is a nop. Returns the written
      files (if any). *)
  ...
  (** [modsrc_impl m ~code ~opts ~requires ~modsrcs src] compile the
      ...
      a nop. Returns the written files (if any). *)
