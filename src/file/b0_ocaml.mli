...
module Lib : sig
  ...
  val make :
    libname:Libname.t -> requires:Libname.t list -> dir:Fpath.t ->
  ...
  (** [make ~libname ~cmis ~cmxs ~cma ~cmxa] is a library named [libname] which
  ...
  val of_dir :
    ...
    requires:Libname.t list -> dir:Fpath.t -> archive:string option ->
    js_stubs:Fpath.t list -> (t, string) result Fut.t
  (** [of_dir m ~clib_ext ~libname ~requires ~dir ~archive] is a library
      named [libname] which requires libraries [requires], with library
...
module Libresolver : sig
 ...
 {[
  ...
  val find : t -> Libname.t -> Lib.t option Fut.t
  (** [find r libname] finds library [libname] in [r]. *)

  val get : t -> Libname.t -> Lib.t Fut.t
  (** [get r libname] gets library [libname] in [r]. The memo of [r]
      fails if the library cannot be found. *)

  val get_list :  t -> Libname.t list -> Lib.t list Fut.t
  (** [get_list r libnames] looks up libraries [libnames] in [r].
      Libraries are returned in the given order and the memo of [r]
      fails if a library cannot be found. *)

  val get_list_and_deps : t -> Libname.t list -> Lib.t list Fut.t
...
(** [lib n ~srcs] is a built unit for a library named [l] made of
    sources [src].
    {ul
    {- [public] indicates whether the library name [name] is made
...
(** {2:metadata Metadata}
    ...
    This is only needed if you need to process units or define
    your own. Both {!exe} and {!lib} set these keys appropriately. *)
...
(** [deprecated_library] on {!library} build unit specifies that
    the {!library} unit name was deprecated for another library
    or that it's use show [`Msg] to the end user. *)
