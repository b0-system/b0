...
(** [libname n] is [n] as an OCaml {{!Lib.Name}library name}.
    This is shortcut for {!Lib.Name.v}. *)
...
(** [ocaml_version b] gets {!Tool.Conf.version} from {!conf}. *)
...
(** [default_lib_resolver] determines the default value of {!lib_resolver}.
    ...
    {- It looks into the build environment via
       {!Lib.Resolver.ocamlpath} and
       {!Lib.Resolver.ocamlfind}
       using the {!B0_build.shared_build_dir} directory of the build.}} *)

val lib_of_unit :
  B0_build.t -> Conf.t -> B0_unit.t -> Lib.t option Fut.t
...
val lib_resolver_build_scope :
  B0_build.t -> Conf.t -> Lib.Resolver.scope
