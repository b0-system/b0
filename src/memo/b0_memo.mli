...
val make_zero :
  ...
  ?win_exe:bool -> ?tool_lookup:tool_lookup -> Env.t ->
  B0_zero.Guard.t -> B0_zero.Reviver.t -> B0_zero.Exec.t -> t
...
val make :
  ...
  ?feedback:([feedback | B0_zero.Exec.feedback] -> unit) -> unit ->
  (t, string) result
...
val spawn' :
  ...
  ?writes:(B0_zero.Op.t -> Fpath.t list) ->
  ?env:Os.Env.t -> ?cwd:Fpath.t -> ?stdin:Fpath.t ->
